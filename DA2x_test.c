...
DA2x_Result test_1(){
  ...
  //
    ...
    f[i++] = DA2x_allocation_size(a0p) == sizeof(int) * 2;  // size is now two ints,  2 * 1/4 -> 0
    f[i++] = DA2x_pop_read(a0p ,&y) && y == 1; // after this pop, the data length will halve again
    f[i++] = DA2x_allocation_size(a0p) == sizeof(int);
...
int main(){
  ...
  DA2x_Result_print(accp ,"in total ");
  return acc.failed;
}
